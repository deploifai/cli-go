...
func (r *AWSCredentialsCreator) getProfiles() ([]interface{}, error) {
	output, err := exec.Command("aws", "configure", "list-profiles").Output()

	if err != nil {
		return []interface{}{}, fmt.Errorf("AWS CLI error: %s. Please make sure you have AWS CLI installed and configured", err.Error())
	}

	// split by \n, remove \r, remove empty strings
	splitStrings := strings.Split(strings.ReplaceAll(string(output), "\r\n", "\n"), "\n")
